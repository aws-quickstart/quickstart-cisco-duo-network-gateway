...
[width="100%",cols="16%,11%,73%",options="header",]
|===
|Parameter label (name) |Default value|Description|Amazon Resource Name (ARN) of the load balancer's SSL certificate
...
(`AdminServerRemoteAccessCIDR`)|`**__Requires input__**`|The allowed CIDR block for remote access to the DNG Admin Server. It is imperative to restrict traffic on the admin server’s port 8443 to authorized network ranges only and not widely internet accessible.|Subdomain for the Admin server
...
(`DomainName`)|`**__Requires input__**`|Fully qualified domain name for the Duo Network Gateway load balancer. If you don't provide a value for ACMSSLCertificateArn, use the HostedZoneID. If you provide a value for this, you must also provide a corresponding HostedZoneID.|Route 53-hosted zone ID of the domain name
...
(`SnapshotName`)|`**__Requires input__**`|Optional name of a snapshot from which you want to restore (leave blank to create an empty cache).|Redis snapshot retention limit
